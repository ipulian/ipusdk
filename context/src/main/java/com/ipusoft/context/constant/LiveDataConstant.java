package com.ipusoft.context.constant;

/**
 * author : GWFan
 * time   : 5/10/21 9:29 AM
 * desc   :
 */

public class LiveDataConstant {
    //音频时长
    public static final String AUDIO_DURATION = "audio_duration";
    //音频进度
    public static final String AUDIO_PROGRESS = "audio_progress";
    //开始播放
    public static final String AUDIO_PLAY = "audio_play";
    //暂停播放
    public static final String AUDIO_PAUSE = "audio_pause";
    //继续播放
    public static final String AUDIO_RESUME = "audio_resume";
    //停止播放音频
    public static final String AUDIO_PLAY_STOP = "audio_play_stop";
    //播放完成
    public static final String AUDIO_PLAY_FINISH = "audio_play_finish";
    //展示播放的悬浮按钮
    public static final String AUDIO_SHOW_PLAY_FLOATING = "audio_show_play_floating";
    //展示SIP外呼的通话弹屏
    public static final String WINDOW_SHOW_SIP_CALL = "window_show_sip_call";
    public static final String SIP_HEART_BEAT = "sip_heart_beat";
    //更新SIP通话状态
    public static final String UPDATE_SIP_CALL_STATUS = "update_sip_call_status";
    //刷新客户列表
    public static final String REFRESH_CUSTOMER_LIST = "refresh_customer_list";
    //刷新线索列表
    public static final String REFRESH_CLUE_LIST = "refresh_clue_list";
    //网络状态改变
    public static final String NETWORK_CHANGED = "network_changed";
    //屏幕状态改变
    public static final String SCREEN_STATUS_CHANGED = "screen_status_changed";
    //外呼方法配置
    public static final String REFRESH_CALL_TYPE_CONFIG = "refresh_call_type_config";
    //上传进度
    public static final String UPLOAD_PROGRESS = "uploadProgress";
    //未读消息
    public static final String UPDATE_UN_READ_COUNT = "update_un_read_count";

    public static final String HOME_PAGE_SEARCH_MEMBER = "home_page_search_member";

    public static final String SEARCH_MEMBER = "search_member";

    public static final String REFRESH_SIP_WINDOW = "refresh_sip_window";

    public static final String SIP_LOGIN_STATUS = "sip_login_status";
}
