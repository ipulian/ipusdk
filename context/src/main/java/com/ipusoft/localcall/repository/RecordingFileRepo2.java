...
public class RecordingFileRepo2 {
    private static final String TAG = "RecordingFileRepo";

    private static class RecordingFileRepoHolder {
        private static final RecordingFileRepo2 INSTANCE = new RecordingFileRepo2();
    }

    public static RecordingFileRepo2 getInstance() {
    ...
    public void queryRecordingFile(Observer<List<File>> observer) {
        ...
        Observable.create((ObservableOnSubscribe<List<File>>) emitter
                        -> emitter.onNext(queryRecordingFile(0, 0)))
    ...
    public List<File> queryRecordingFile(long beginTime) {
        return queryRecordingFile(1, beginTime);
    }

    public List<File> queryRecordingFile(int isAll, long beginTime) {
        ...
        if (tempFile != null && tempFile.exists() && tempFile.isDirectory()) {
            ...
            if (children != null && children.length != 0) {
                if (isAll == 1) {
                    for (File f : children) {
                        if (f != null && f.exists() && f.isFile() && judgeRecordFile(f) && judgeFileContainsNumber(f)
                ...
                /**
                 * 有些机型的录音文件不包含手机号，所以如果找不到录音文件，就把文件名的判断去掉
                 */
        ...
        if (ArrayUtils.isEmpty(list)) {
            list = traverseSDCard(isAll, beginTime, true);
        } else {
        ...
        if (ArrayUtils.isNotEmpty(list)) {
            ...
            list = traverseSDCard(isAll, beginTime, false);
        }
     ...
     * @return
     */
    private ArrayList<File> traverseSDCard(int isAll, long beginTime, boolean judgeFileContainsNumber) {
        ArrayList<File> list = new ArrayList<>();
            ...
            if (children == null || children.length == 0) {   //若该目录是空目录，则略过该目录
                ...
                for (File file : children) {
                    if (file.isDirectory()) {
                        ...
                        if (isAll == 1) {
                            if (judgeFileContainsNumber) {
                                if (file.exists() && file.isFile() && judgeRecordFile(file) && judgeFileContainsNumber(file)
